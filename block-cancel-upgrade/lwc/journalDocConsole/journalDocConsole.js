...
export default class JournalDocConsole extends LightningElement {
  ...
  _normalizeDocs(list) {
    ...
    return list.map(r => {
      ...
      return {
        ...
        // NEW: block approval
        isApprovalBlocked: r.isApprovalBlocked === true,
        showBlockApprovalCheckbox: (status || '').toLowerCase() !== 'approved'
      };
    });
  }
  ...
  // NEW: Handle Block Approval checkbox change
  async onBlockApprovalChange(evt) {
    const id = evt?.target?.dataset?.id;
    const checked = evt?.target?.checked;
    if (!id) return;
    try {
      await updateBlockApproval({ docId: id, isBlocked: checked });
      this.docs = this.docs.map(d => d.id === id ? { ...d, isApprovalBlocked: checked } : d);
      this._flashTypeSaved(id); // reuse same success flash
      this._setSuccess(checked ? 'Approval blocked.' : 'Approval unblocked.');
    } catch (e) {
