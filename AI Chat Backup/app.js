...
async function bootImpersonationIfPresent() {
  ...
  if (!token) return; // Not an impersonation link
  ...
  try {
    ...
    // Store session and journal info
    sessionToken = json.session;
    selectedJournalId = json.journalId;
    selectedJournalName = json.journalName;
    impersonationMode = true;
    impersonationAllowApprove = json.allowApprove || false;
    
    // Set identifier mode (impersonation doesn't use email/phone)
    identifierType = 'impersonation';
    identifierValue = json.journalName;
    ...
    showImpersonationError(err.message || 'Invalid or expired access link');
  }
}
...
if ($('confirmCancel2')) $('confirmCancel2').onclick=()=>{
  hide($('confirmModal'));
  // Restore approve button visibility for next time
  if ($('confirmApprove')) show($('confirmApprove'));
  modalResolve(false);
};
...
function openBlockedApprovalModal(){
  if ($('confirmText')) $('confirmText').innerHTML = 
    `<strong>${_t_safe('BLOCKED_APPROVAL_HEADER')}</strong><br><br>${_t_safe('BLOCKED_APPROVAL_BODY')}`;
  if ($('confirmApprove')) hide($('confirmApprove')); // Hide approve button
  if ($('confirmCancel2')) $('confirmCancel2').textContent = _t_safe('CLOSE') || 'Luk';
  show($('confirmModal'));
}

async function tryApprove(ids){
  // Check if any documents have approval blocked
  const blockedDocs = docs.filter(d => ids.includes(d.id) && d.isApprovalBlocked === true);
...
// Boot impersonation mode if ?impersonate= parameter is present
bootImpersonationIfPresent();
